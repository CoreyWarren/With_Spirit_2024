...
public class canvas_soulscript : MonoBehaviour {
    ...
    private float soulToEnergyTimer; // Timer for Soul-to-Energy effect
    private float soulToEnergyTimerMax = 12; // Duration of the effect
    ...
    // Public to ensure they can be accessed by other objects.
    public static bool uiSoulTouch;
    public static bool uiPainTouch;
    public static bool uiSoulToEnergy;
	...
	void Update () {
        ...
        if (painTimer == 0 && blinkTimer == 0)
        {
            ...
            if (uiSoulToEnergy)
            {
                soulToEnergyTimer = soulToEnergyTimerMax;
                transform.localScale = size2;
                soulText.color = colors[2]; // Green color for conversion effect
                uiSoulToEnergy = false; // Reset the trigger
            }

            if (soulToEnergyTimer > 0)
            {
                soulToEnergyTimer--;

                // Oscillating scale effect during the conversion
                float pulse = Mathf.PingPong(Time.time * 2, 0.2f); // Scale oscillates by ±0.1
                transform.localScale = size2 + new Vector2(pulse, pulse);

                // Color shift between green and blue during effect
                soulText.color = Color.Lerp(colors[2], colors[3], Mathf.PingPong(Time.time * 2, 1));
            }
            else if (soulToEnergyTimer <= 0 && blinkTimer == 0 && painTimer == 0)
            {
                // Reset to default appearance when the effect ends
                transform.localScale = size1;
                soulText.color = defaultColor;
            }

        }// end of if (!uiPainTouch && !uiSoulTouch)
