...
public class PlayerController : MonoBehaviour {
    ...
    [SerializeField]
    private AudioClip transferSoulToEnergySound; 
    ...
    [SerializeField]
    private int[] energyWeaponCosts = new int[4];
    [SerializeField]
    ...
    // Turn excess Souls into Energy
    [SerializeField]
    private float   timeToBeginSoulToEnergy = 3.0f;
    [SerializeField]
    private float   timeIntervalSoulToEnergy = 1.0f;
    [SerializeField]
    private int     energyPerSoulForSoulToEnergy = 30;
    [SerializeField]
    private int     soulThresholdSoulToEnergy = 3;
    [SerializeField]
    private float   timeSinceLastSoulToEnergy = 0f;
    [SerializeField]
    private bool    isConvertingSoulsToEnergy = false;
    ...
    void Update()
    {
        ...
        if (pauser1.paused == false)
        {
            ...
            else
            {
                ...
                jumptime -= Time.fixedDeltaTime * 10;
                //Sparks
                sparktimer--;
                if (sparktimer < 1)
                {
                ...
                // SOUL TO ENERGY CONVERSION:
                // Check if the player has enough souls to start conversion
                if (playerStats.playerSouls >= soulThresholdSoulToEnergy)
                {
                    isConvertingSoulsToEnergy = true;
                }
                else
                {
                    isConvertingSoulsToEnergy = false;
                }

                // If conversion is enabled, process the conversion based on the interval
                if (isConvertingSoulsToEnergy && timeSinceLastSoulToEnergy < timeIntervalSoulToEnergy)
                {
                    timeSinceLastSoulToEnergy += Time.fixedDeltaTime;
                }
                if (timeSinceLastSoulToEnergy >= timeIntervalSoulToEnergy && (playerStats.playerEnergy + energyPerSoulForSoulToEnergy <= Player_Stats_Storage.playerEnergyMax) )
                {
                    ConvertSoulToEnergy();
                    timeSinceLastSoulToEnergy = 0f;
                }
    ...
    public void soulTouchCheck()
    {
        ...
        {
            if (playerStats.playerHealth == Player_Stats_Storage.playerHealthMax)
            {
    ...
    public void painHealthAnim()
    {
        ...
        else if (playerStats.playerHealth == Player_Stats_Storage.playerHealthMax - 1)
        {
            GetComponent<SpriteRenderer>().color = yellow;
        }
        else if (playerStats.playerHealth <= Player_Stats_Storage.playerHealthMax - 3)
        {
        ...
        if (sprBT == 0 && playerStats.playerHealth == Player_Stats_Storage.playerHealthMax)
            sprBT = sprBTmax1;
    }

    private void ConvertSoulToEnergy()
    {
        // Deduct one soul and add the corresponding energy
        playerStats.playerSouls--;
        playerStats.playerEnergy += energyPerSoulForSoulToEnergy;
        canvas_soulscript.uiSoulToEnergy = true;

        // Play the sound effect for conversion
        if (audio1 != null && transferSoulToEnergySound != null)
        {
            audio1.PlayOneShot(transferSoulToEnergySound);
        }

        Debug.Log("Converted 1 soul into " + energyPerSoulForSoulToEnergy + " energy.");
        Debug.Log("Remaining Souls: " + playerStats.playerSouls + ", Total Energy: " + playerStats.playerEnergy + ".");
    }
